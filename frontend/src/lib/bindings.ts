...
export type JobTask =
  | { Scan: { path: string; retry_count: number } }
  | {
      Fix: {
        path: string;
        release_id: string;
...
export type ProceduresLegacy = {
  queries:
    | { key: "config_read"; input: null; result: string }
    | {
        key: "queue_info";
        input: null;
        result: { tasks: JobTask[]; running_count: number };
      }
    | {
        key: "scan_log";
        input: { limit: number; page: number; success: boolean | null };
        result: [ScanLog[], number];
      };
  mutations:
    ...
    | { key: "queue_clear"; input: null; result: null }
    | { key: "scan"; input: { path: string }; result: null }
    | { key: "scan_all"; input: null; result: null }
    | { key: "scan_log_clear"; input: { clear_failed: boolean }; result: null };
  subscriptions: never;
};
...
export type ScanLog = {
  ...
  success: boolean;
  message: string | null;
  old_metadata: Metadata | null;
  new_metadata: Metadata | null;
  source_path: string;
...
export type Procedures = {
  ...
  fix: {
    kind: "mutation";
    input: { target_path: string; release_id: string; recording_id: string };
    output: null;
    error: Error;
  };
  fix_failed: {
    kind: "mutation";
    input: { source_path: string; release_id: string; recording_id: string };
    output: null;
    error: Error;
  };
  ...
  queue_info: {
    kind: "query";
    input: null;
    output: { tasks: JobTask[]; running_count: number };
    error: Error;
  };
  scan: {
    kind: "mutation";
  ...
  scan_log: {
    kind: "query";
    input: { limit: number; page: number; success: boolean | null };
    output: [ScanLog[], number];
    error: Error;
  };
  scan_log_clear: {
    kind: "mutation";
    input: { clear_failed: boolean };
    output: null;
    error: Error;
  };
};
